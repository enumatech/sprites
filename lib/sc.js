...
const {
    ...
    transpose, all, either, isNil, equals
} = require('ramda')
...
const SC = module.exports = {
    ...
    serialize(channelState) {
        ...
        //         "address", // recipient (FIXME should be address)
        //         "uint", // amount
        //         "uint", // expiry
        //     ],
        //     SC.vector(channelState)
        // )

        let i = 0
        const ch = SC.vector(channelState)
        ...
        return "0x" + [
            ...
            enc("address", ch[i++]), // recipient // FIXME should be address
    ...
    checkAvailSigs(ch) {
        const h = hashPersonalMessage(toBuffer(SC.serialize(ch)))
    ...
    validIdx: idx => contains(idx, [0, 1]),

    validateIdx: curry((idx, ch) => {
        assert(SC.validIdx(idx),
    ...
    balance: curry((idx, ch) => {
        ...
        return deposit + credit - withdrawal - withdrawn
    })
}
