...
const pay = curry((amt, sprites) => Sprites.transition([
        ['credit', Sprites.otherIdx(sprites), amt]],
    sprites))

const invoice = curry((amt, sprites) => Sprites.transition([
        ['credit', Sprites.ownIdx(sprites), amt],
        ['withdraw', Sprites.ownIdx(sprites), amt]],
    sprites))

const withdraw = curry((amt, sprites) => Sprites.transition([
        ['withdraw', Sprites.ownIdx(sprites), amt]],
    sprites))

const confirmWith = curry(async (them, transition, us) => {
        ...
        describe('remaining amounts cooperatively after off-chain payment', () => {
            it('using primitive methods', async () => {
                ...
                const Alice1 = await threadP(Alice,
                    ...
                    Sprites.approve(amt),
                    Sprites.deposit(amt),
                    confirmWith(Bob, pay(3)),
                    confirmWith(Bob, withdraw(amt - 3)),
                    Sprites.update,
                    Sprites.withdraw,
                    Sprites.channelState,
                    Sprites.tokenBalance)
            ...
            it('using combined methods', async () => {
                ...
                const Alice2 = await threadP(Alice1,
                    confirmWith(Bob1, withdraw(amt - 3)),
                    Sprites.updateAndWithdraw,
                    Sprites.channelState,
                    Sprites.tokenBalance)
            ...
            it('takes multiple transactions', async () => {
                ...
                const payToBob = async (someone) => {
                    ...
                    await threadP(someone,
                        confirmWith(Bob, pay(3)),
                        confirmWith(Bob, withdraw(7)))

                    await threadP({...Bob, chId},
                        confirmWith(someone, withdraw(3)))
