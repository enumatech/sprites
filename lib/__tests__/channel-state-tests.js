...
describe('Channel State', function () {
    ...
    describe('.creditAndWithdraw', () => {
        ...
        it('amount is transferred and ready to withdraw', () => {
        ...
        it('subsequent calls are cumulative', () => {
            const ch0 = thread(ch,
                creditAndWithdraw(0, 3),
                creditAndWithdraw(0, 1))
            expect(ch0).toMatchObject({
                credits: [3 + 1, -3 - 1],
                withdrawals: [10 + 3 + 1, 0],
                round: ch.round + 2,
            })

            const ch1 = thread(ch,
                creditAndWithdraw(1, 3),
                creditAndWithdraw(1, 1))
            expect(ch1).toMatchObject({
                credits: [-3 - 1, 3 + 1],
                withdrawals: [0, 9 + 3 + 1],
                round: ch.round + 2,
