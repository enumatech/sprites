...
const Sprites = {
    ...
    async testEthersDeploy(chain) {
        ...
        const [preimageManager, token] = await Promise.all([
            deploy(PreimageManager),
            deploy(ERC20Token, "Test DAI", "DAI", 0, new BigNumber(`10`).pow(6),
                await Deployer.getAddress())])
        const reg = await deploy(SpritesRegistry, preimageManager.address)

        return Sprites.make({
            preimageManager, reg, token,
            ACTOR_NAME: 'Deployer',
        })
    },

    randomTestClient: curry(async (NAME, s) => {
        const {preimageManager, reg, token} = s
        const wallet = ethers.Wallet.createRandom().connect(reg.provider)
        wallet.NAME = NAME    // Used by `util.inspect`

        const sendEther = async (who) =>
            reg.signer.sendTransaction({
                to: await who.getAddress(),
                value: parseEther(`10`)
            })

        const sendToken = async (who) =>
            token.transfer(await who.getAddress(), new BigNumber(`1000`))

        await sendEther(wallet)
        await sendToken(wallet)

        const connectContract = (baseContract) => {
            const c = baseContract.connect(wallet)
            c.NAME = baseContract.NAME    // Preserve contract name if any
            return c
        }

        return Sprites.make({
            ACTOR_NAME: NAME,
            ...map(connectContract, {preimageManager, reg, token})
        })
    }),
    ...
    async balances(s) {
        ...
        return {
            ...
            DAI: await token.balanceOf(await signer.getAddress()),
            who: signer
        }
    },
