...
const Sprites = {
    ...
    async testDeploy({web3Provider}) {
        ...
        const reg = await deploy(SpritesRegistry, preimageManager.options.address)
        ...
        return {
            ...
            ...map(path(['options', 'address']), {preimageManager, reg, token})
        }
    },
    ...
    async sign(s) {
        const sig = await s.sign(SC.serialize(s.channel))
        /*
        ...
        * */
        ...
        sig.ch = assoc(inspect.custom, SC.inspector, s.channel)
    ...
    channelState: async (s) => {
        ...
        const inspectableChannel = assoc(inspect.custom, SC.inspector, channel)
    ...
    create: curry(async function (otherAddress, s) {
        const tx = await s.reg
            .create(otherAddress, s.token.options.address)
    ...
    approve: curry(async function (amount, s) {
        const tx = await s.token
            .approve(s.reg.options.address, amount)
    ...
    createWithDeposit: curry(async function (otherAddress, amount, s) {
        const tx = await s.reg
            .createWithDeposit(otherAddress, s.token.options.address, amount)
    ...
    updateAndWithdraw: curry(async function (s) {
        ...
        const sig = ch.sigs[Sprites.otherIdx(s)]
        const tx = await s.reg
            .updateAndWithdraw(...SC.vector(ch), sig)
    ...
    cmd: {
        ...
        apply: (s) => {
            ...
            const ch1 = SC[Sprites.cmd.allowed(name)](...params, ch0)
